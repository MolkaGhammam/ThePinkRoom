...
--
-- Helpers live in the public schema (auth schema is owned by Supabase platform):
--   public.get_salon_id()  -> the calling user's salon_id (or NULL)
--   public.get_user_role() -> the calling user's role string (or NULL)
--
-- General rule: a row is visible only if salon_id = public.get_salon_id() AND deleted_at IS NULL.
...
create policy "salons: read own salon"
  ...
  using (id = public.get_salon_id() and deleted_at is null);

create policy "salons: admin updates own salon"
  on public.salons for update
  using (id = public.get_salon_id() and public.get_user_role() = 'admin')
  with check (id = public.get_salon_id() and public.get_user_role() = 'admin');
...
create policy "salon_closures: read own salon"
  ...
  using (salon_id = public.get_salon_id() and deleted_at is null);

create policy "salon_closures: admin writes"
  on public.salon_closures for all
  using (salon_id = public.get_salon_id() and public.get_user_role() = 'admin')
  with check (salon_id = public.get_salon_id() and public.get_user_role() = 'admin');
...
-- A user can update their own row (e.g. profile, locale).
...
create policy "clients: any user writes"
  ...
  using (salon_id = public.get_salon_id())
  with check (salon_id = public.get_salon_id());
...
create policy "appointments: staff update own"
  ...
  using (salon_id = public.get_salon_id() and staff_user_id = auth.uid())
  with check (salon_id = public.get_salon_id() and staff_user_id = auth.uid());
